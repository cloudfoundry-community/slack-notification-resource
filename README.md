...
- when `text` and `attachments` are omitted, and...
...
- when `text` is present and `attachments` is omitted, and `text` evaluates to empty string after variable interpolation:
  > *(missing notification text)*
